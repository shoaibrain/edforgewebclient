      ...
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-8 w-8 rounded-lg sidebar-transition">
                <AvatarImage src={user.avatar} alt={user.name} />
                <AvatarFallback className="rounded-lg bg-primary text-primary-foreground font-semibold">
                  {user.name.split(' ').map(n => n[0]).join('').toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div className="grid flex-1 text-left text-sm leading-tight sidebar-transition">
                <span className="truncate font-medium">{user.name}</span>
                <span className="truncate text-xs">{user.email}</span>
              </div>
              <ChevronsUpDown className="ml-auto size-4 sidebar-transition" />
            </SidebarMenuButton>
          </DropdownMenuTrigger>
          <DropdownMenuContent
